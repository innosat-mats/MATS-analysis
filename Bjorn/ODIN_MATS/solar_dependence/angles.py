...
ds_night_all=ds_night_all.sel(time = slice("2023-02-01", "2023-03-01")).dropna(dim='time')
ds_day_all=ds_day_all.sel(time = slice("2023-02-01", "2023-03-01")).dropna(dim='time')
...
for dayi in range(1,5): # for every day 
    ...
    if (len(dftop.index) > 0) and (len(ds_day.time) > 0):
        ...
        # NOW MATSi contains all times on a given day within ten minutes of every Odin Measurement... 
