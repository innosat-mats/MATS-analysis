...
def gradientmatrix(items,airglowlim, auroralim):
    ...
    for rowindex in range(airglowlim,matrix.shape[0]):
        ...
        if rowindex >= auroralim:
            ...
            valid_indices = np.where(y < 350)[0]
...
def get_strips(items, numdays,filedate):
    ...
    return aurorastrips
# %%
testitems = pd.read_pickle(r'C:\Users\ceona\Documents\GitHub\MATS-analysis\MATS-analysis\Ceona\26feborb12')
...
"""
    ...
    for rowindex in range(airglowlim,matrix.shape[0]):
        y = matrix[rowindex,:] 
        valid_indices = np.where(y < 370)[0]
        """
        if len(nonvalid_indices) > 0 :
            for elem in nonvalid_indices:
                y[elem] = y[elem-1]
        """
        y_filtered = y[valid_indices]
        z_filtered = np.arange(0, len(y_filtered))
        z = np.arange(0,len(y))
        mymodel = np.poly1d(np.polyfit(z_filtered, y_filtered, 3))
        y_regf = mymodel(z_filtered)
        y_reg = mymodel(z)
        newmatrix[rowindex,:] = y-y_reg
        if rowindex == airglowlim or rowindex == airglowlim + 30: 
            axs[0].plot(y_filtered, label = f"Row {rowindex}")
            axs[0].plot(y_regf, label = f"y_regf, Row {rowindex}")
            axs[0].plot(y_filtered-y_regf, label = f"Row {rowindex} Diff")
...
# %%
def Main():
    start_time = DT.datetime(2023,2,22,00,0,0)
    stop_time = DT.datetime(2023,3,1,00,0,0)
    numdays = stop_time-start_time
    filedate = 'feb4W'
    items = pd.read_pickle(r'C:\Users\ceona\Documents\GitHub\MATS-analysis\MATS-analysis\Ceona\22to28febIR1')
