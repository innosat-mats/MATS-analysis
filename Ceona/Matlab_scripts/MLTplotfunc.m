function fig = MLTplotfunc(type,stripsNH,stripsSH,peaksNH,peaksSH)
    fig = figure ; 
    if type == 1
        sgtitle('MLT plot of peak points, April','fontweight','bold',fontsize=16)
        ...
        subtitle('North Hemisphere','FontWeight','bold',FontSize=12);
        ...
        subtitle('South Hemisphere','FontWeight','bold',FontSize=12);

    end

    if type == 2
        sgtitle('Peak points and path')
        
        %%%% North Hemisphere polar plot
        subplot(1,2,1);
        
        %plot settings
        s1 = polarplot(stripsNH.MLT *(pi/12),stripsNH.Mlat,'.',MarkerSize=0.05) ; 
        hold on
        p1 = polarscatter(peaksNH.MLT*(pi/12),peaksNH.Mlat,[],peaksNH.kp,'filled');
        set(gca,"CLim",[1 6])  
        colorbar
        ...
        subtitle('North Hemisphere');
        
        %%%% South Hemisphere polar plot
        subplot(1,2,2) ;
        
        s2 = polarplot(stripsSH.MLT*(pi/12),stripsSH.Mlat,'.',MarkerSize=0.05) ;
        hold on
        p2 = polarscatter(peaksSH.MLT*(pi/12),peaksSH.Mlat,[],peaksSH.kp,'filled');
        set(gca,"CLim",[1 6])  
        colorbar
