...
#starttime = datetime(2023, 2, 17, 2, 10)
#stoptime = datetime(2023, 2, 17, 2, 50)
starttime=datetime(2023,3,23,22,0)
stoptime=datetime(2023,3,23,23,35)
...
offset = 0
num_profiles = 370  # use 50 profiles for inversion
...
for i in range(0, num_profiles, 10):
    ...
    # Change calibration to new Jörg
    p1 *= 11.4 / 10.5
    p2 *= 1
    p3 *= 19.8 / 19.1
    ...
    plt.semilogx(
        p1, z1, p2, z2, p3, z3, p4, z4, p3 - p3stray, z3, p4 - p4stray, z4
    )  # ,puv1,z1,puv2,z2)
    # plt.semilogx(p1,z1,p2,z2,p3,z3,p4,z4,p3,z3,p4,z4) #,puv1,z1,puv2,z2)
    ...
    plt.title(
        "TP sza {:5.2f} TP ssa {:5.2f}  TP lat {:5.2f} TP lon {:5.2f}".format(
            ir3.TPsza[i], ir3.TPssa[i], ir3.TPlat[i], ir3.TPlon[i]
        )
    )
    # plt.xlim([1e15, 3e17])
    # plt.figure()
    ...
    mjd = mjdtools.utc2mjd(
        ir3.EXPDate[i].year,
        ir3.EXPDate[i].month,
        ir3.EXPDate[i].day,
        ir3.EXPDate[i].hour,
        ir3.EXPDate[i].minute,
        ir3.EXPDate[i].second,
    )
    ...
    plt.legend(["IR1", "IR2", "IR3", "IR4", "IR3c", "IR4c", "s3", "s4"])
    plt.show()
    plt.figure()
    meas = np.array([p3 - p3stray, p4 - p4stray]).T
    model = irr * radiance.T
    strays.append(np.array([p3 - model[:, 1], p4 - model[:, 1]]).T)
    ...
    plt.show()
    plt.figure()
    plt.plot(p3 - p3stray - model[:, 0], tanalts_km * 1000, label="ir3c-s3")
    plt.plot(p4 - p4stray - model[:, 1], tanalts_km * 1000, label="ir4c-s4")
    plt.plot(p3 - model[:, 0], tanalts_km * 1000, label="ir3-s3")
    plt.plot(p4 - model[:, 1], tanalts_km * 1000, label="ir4-s4")
    plt.legend()
    plt.title("Ir - model")
...
# %%
plt.figure()
plt.scatter(ssas,szas,c=np.array(scales)[:,1])
plt.xlabel('SSA')
plt.ylabel('SZA ')
# %%
