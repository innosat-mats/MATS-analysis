...
start_time = DT.datetime(2022, 12, 21, 5, 0, 0)
stop_time = DT.datetime(2022, 12, 21, 6, 0, 0)
...
STACKED_IM = np.zeros((nb_bin,a,b))
NB_IM = np.zeros((nb_bin,a,b))
data = []
...
for i in tqdm(range(nb_bin)):
        ...
        for k in range(a):
            for l in range(b):
                if sza_min <= sza_points[j,k,l] and sza_points[j,k,l] <= sza_max :
                    ...
                    STACKED_IM[i,k,l] += im_points[j,k,l]
                    NB_IM[i,k,l] += 1
    ...
    for k in range(a):
        for l in range(b):
            if NB_IM[i,k,l] == 0:
               STACKED_IM[i,k,l] = None
            else :
                STACKED_IM[i,k,l] = STACKED_IM[i,k,l]/NB_IM[i,k,l]
...
mask = stacked_im < 7000 # arbitrary
correction = -(stacked_im-np.nanmean(im_points_night))*(1-mask)

im_points_corrected = np.zeros_like(im_points)
for j in range(n):
    for k in range(a):
        for l in range(b):
            if im_points_night[j,k,l] != None:
                im_points_corrected[j,k,l] = im_points_night[j,k,l] + correction[k,l]
...
for i in range(len(DataWindow)):
    for j in range(n):
        ...
        if df1a.iloc[j]['WDWInputDataWindow'] == DataWindow[i]:
            ...
            for k in range(a):
                for l in range(b):
                    MEAN_PIX[i].append(im_points_corrected[j,k,l])
                    if sza_points[j,k,l] > ang_lim :
                        TMP95.append(im_points_corrected[j,k,l])
                    if im_points_corrected[j,k,l] > max_val and sza_points[j,k,l] > ang_lim :
                            max_val = im_points_corrected[j,k,l]
                            maxk = k
                            maxl = l
            print(DataWindow[i])
            print(maxk,maxl,max_val)
            print(len(TMP95))
            ARTIFACT[i].append(im_points_corrected[j,10,17])
            MEAN_SZA[i].append(np.mean(sza_points[j,:,:]))
            if len(TMP95) > 0:
                VAL95[i].append(np.percentile(TMP95,95))
            if max_val != 0:
