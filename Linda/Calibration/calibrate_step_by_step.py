...
def plot_calib_step(step1,step2,title,error,divide=False, clim=None):

    error = np.zeros(np.shape(step1))

    fig, axs = plt.subplots(1,3)
    fig.suptitle(title, fontsize=16)
    sc = axs[0].imshow(step1, vmin=np.mean(step1)-np.std(step1), vmax=np.mean(step1)+np.std(step1),origin='lower')
    ...
    if divide:
        change=step2/step1
        sc = axs[2].imshow(change,vmin=np.median(change)-np.std(change), vmax=np.median(change)+np.std(change),origin='lower')
    else:
        change=step2-step1
        sc = axs[2].imshow(change,origin='lower')
