...
def add_node_labels(ds):
    """
    Add node labels (ascending/descending) to dataset based on latitude changes.
    
    Parameters:
    - ds3d (xr.Dataset): Dataset with 'latitude' and 'img_time' dimensions
    
    Returns:
    - xr.Dataset: Dataset with added 'node' variable
    """
...
file_path = "/Users/lindamegner/MATS/MATS-retrieval/data/Lukas1DRegressionOfL1b/linear_L2_2023_03_05-15.nc"
#read in Björns data
file_path_airIR1='/Users/lindamegner/MATS/MATS-retrieval/data/Bjorns1dAirglow/IR1_2023-03f.nc'
# Load the dataset
...
node='ascending'
ds3d=ds3d.where(ds3d['node']==node, drop=True)




#%%
# take the mean of VER and plot as function of latitude and altitude (latitude is not a coordinate yet)
...
if slowway:
    ...
    for ibin in range(len(lat_bins)-1):
        lat_min = lat_bins[ibin]
        lat_max = lat_bins[ibin+1]
        ds3d_latbin = ds3d.where((ds3d['latitude'] >= lat_min) & (ds3d['latitude'] < lat_max), drop=True)
        mean_VER = ds3d_latbin['VER'].mean(dim='img_time')

        if ibin == 0:
            VER_by_lat = mean_VER.expand_dims(lat_bin=[lat_labels[ibin]])
        else:
            VER_by_lat = xr.concat([VER_by_lat, mean_VER.expand_dims(lat_bin=[lat_labels[ibin]])], dim='lat_bin')
    # Plot
    VER_by_lat.plot(x='lat_bin', y='alt_coord', cmap='inferno', robust=True)
    plt.title('Mean VER vs Latitude and Altitude '+ str(node)+' node')
    ...
    #This does the same thing as above but using groupby_bins instead of looping through latitude bins
    ...
    plt.ylabel('Altitude')
    plt.title('Mean VER binned by latitude (1°) and altitude'+' '+ str(node)+' node')
    plt.tight_layout()
    plt.show()
...
#%%
# Plot and Compare to Björns data
dsairIR1=xr.open_dataset(file_path_airIR1)
# Display basic info about the dataset
#CHANGE NAME OF VARIABLEs
dsairIR1=dsairIR1.rename({'alt':'alt_coord'})
dsairIR1=dsairIR1.rename({'TPlat':'latitude'})
dsairIR1=dsairIR1.rename({'TPlon':'longitude'})
dsairIR1['GMT'] = xr.apply_ufunc(
    get_utc_hour,
    dsairIR1['img_time'],
    vectorize=True,
    output_dtypes=[float]
...
# Get the time range and color scale limits from ds3d
#time_min = ds3d['img_time'].min().values
#time_max = ds3d['img_time'].max().values
time_min = "2023-03-13T08:00:43"
time_max = "2023-03-13T09:30:43"

fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(10, 8), constrained_layout=True)

im = ds3d['VER'].sel(img_time=slice(time_min, time_max)).plot(x='img_time', y='alt_coord', cmap='inferno', robust=True, ax=ax1)
ax1.set_title("MATS Data")
# get color scale limits from first plot
climits = im.get_clim()


dsairIR1.sortby('img_time')['VER'].sel(img_time=slice(time_min, time_max)).plot(
    x='img_time', y='alt_coord', cmap='inferno', vmin=climits[0], vmax=climits[1], ax=ax2
)
ax2.set_title("Björn's Data")

# plot the latitude as function of time
dssub=ds3d.sel(img_time=slice(time_min, time_max))
ax3.plot(dssub['img_time'], dssub['latitude'], marker='.', linestyle='None')
ax3.set_title("MATS Latitude")
...
# Convert ds3d['img_time'].min() to format "2023-03-13T08:00:43"
...
for altitude in altituderanges:
    ...
    for s in range(-3, 4, 1):
        ...
        for n in range(1, 3, 1):  # harmonic of solar day
...
# #0.4 , this pattern seems to match between mats and cmtm 
#n=2
#s=1

# # 0.4 clear pattern but not matching well
# n=2
# s=3

# #0.6 , this pattern seems to match between mats and cmtm
n=1
s=2

# #0.6 Is this even a wave?
# n=1
# s=0
...
#reset index so that it goes from 0 to len-1
results_to_plotM=results_to_plotM.reset_index(drop=True)
results_to_plotC=results_to_plotC.reset_index(drop=True)

figM, axsM = plt.subplots(max(len(results_to_plotM),2), 1, figsize=(10, 10), tight_layout=True)
figC, axsC = plt.subplots(max(len(results_to_plotC),2), 1, figsize=(10, 10), tight_layout=True)
