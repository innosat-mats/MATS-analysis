...
if doIR3:
    ...
    channel='IR3'
    directoryIR3='/Users/lindamegner/MATS/retrieval/Calibration/FinalFinalSept2021/BinningFlatfieldsIR3_210910/'
    protocolIR3='readprotocol_IR3.txt'
    
    read_from = "imgview"
    df_ir3= readprotocol(directoryIR3 + protocolIR3)
    for direction in directions:
        if direction == 'horizontal':
            imageitm_B = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[0], read_from)
            imageitm_SB = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[1], read_from)
            imageitm_D = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[2], read_from)
            imageitm_SD = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[3], read_from)

            image= imageitm_B['IMAGE']-imageitm_SB['IMAGE']-(imageitm_D['IMAGE']-imageitm_SD['IMAGE'])
            images_horizontal[channel] = image
        else: #vertical
            imageitm_B = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[4], read_from)
            imageitm_SB = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[5], read_from)
            imageitm_D = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[6], read_from)
            imageitm_SD = read_CCDitem_rac_or_imgview(directoryIR3, df_ir3.PicID.iloc[7], read_from)
...
for channel in channels:
    ...
    plt.show()
    #Save the plot
    fig.savefig('../output/compare_different_flatfields_'+channel+'.png')
