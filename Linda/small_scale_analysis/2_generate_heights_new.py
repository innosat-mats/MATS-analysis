...
def generate_heights_save(CCDs,part):
    ...
    if ascending:
        CCDs.to_pickle(f'{main_folder}/CCDSEL{CCDno}/alts_80to90/{monthstring}/{monthstring}_part{part}.pk1')
    else:
        CCDs.to_pickle(f'{main_folder}/CCDSEL{CCDno}/alts_80to90/{monthstring}_desc/{monthstring}_part{part}.pk1')
...
for month in [2]:
    for day_couple in range(len(day_couples)):
        for CCDno in [2, 3]:
            ...
            pathname = f'{main_folder}/CCDSEL{CCDno}'
            ...
            # to check path
            outpathname = f'{main_folder}/CCDSEL{CCDno}/alts_80to90/{monthstring}'

            # Check whether the specified path exists or not
            isExist = os.path.exists(outpathname)
            if not isExist:
