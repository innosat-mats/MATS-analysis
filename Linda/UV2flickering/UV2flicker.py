...
CCDitems_flip = dataframe_to_ccd_items(df[:50])
#%%
# Calibrate the images
def calibration_steps(CCDitem, instrument, flippit='nonflipped'):
    ...
    if flippit == 'flipped':
        CCDitem["image_desmeared"] = np.flipud(CCDitem["image_desmeared"])
...
for CCDitem in CCDitems:
    CCDitem = calibration_steps(CCDitem, instrument, 'nonflipped') 
for CCDitem in CCDitems_flip:
    CCDitem = calibration_steps(CCDitem, instrument, 'flipped')

#add the flipped version to the nonflipped version
for i, CCDitem in enumerate(CCDitems):
    CCDitems[i]['ImageCalibFlipped'] = CCDitems_flip[i]['ImageCalib']
...
# %%
df['ColumnMean'] = df['ImageCalib'].apply(lambda x: np.mean(x, axis=1))
df['ColumnMeanFlipped'] = df['ImageCalibFlipped'].apply(lambda x: np.mean(x, axis=1))


#%%
# Create the directory

directory = os.path.join(data_folder, 'uv2flicker')
...
for index, CCD in df.iterrows():
    #plot_image(CCD, outpath=directory+'/', image_field='ImageCalib')
    plt.figure()
    plt.plot(CCD['ColumnMean'], label='Nonflipped')
    plt.plot(CCD['ColumnMeanFlipped'], label='Flipped')
    plt.ylim(-50, 1000)
    plt.legend() 
    plt.tight_layout()
    plt.savefig(directory+'/columnmean_'+str(index)+'.png')
     #CCD['ImageCalibrated']

#%%


generate_gif(directory+'/', data_folder+'uv2flicker_'+'orbit.gif')
